...
* 验证 ID 令牌（用于从客户端应用向后端服务器传递已登录用户）。


注：Wilddog Server SDK 中包含的 ID 令牌认证方法不能认证你用 Wilddog Server SDK 创建的自定义令牌。

jar包下载地址:
https://cdn.wilddog.com/sdk/java/v2/wilddog-auth-sdk-2.0.1.jar

md5:de55d60b62d11b281e884738edfbe24c
...
```java
 ...
 developerClaims.put("claims3", "zidane");

 CustomTokenGenerator a = new CustomTokenGenerator(secret);//超级密钥
 TokenOptions options = new TokenOptions();
 options.setAdmin(true);
 String token = a.createCustomToken("some-uid", developerClaims, options);
```
