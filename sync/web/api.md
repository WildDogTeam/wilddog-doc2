...
var rootRef = wilddog.database().ref();
//Good, 我们已经创建了一个野狗客户端。
...
wilddog.initializeApp(config);
var ref = wilddog.database().ref("city");
...
wilddog.initializeApp(config);
var ref = wilddog.database().ref("/city/Beijing");
...
wilddog.initializeApp(config);
var ref = wilddog.database().ref("users");
...
把数据写到当前位置，类似set,不同之处是需要指定一个优先级。默认排序按照优先级排序。(参考排序规则的 [orderByPriority](/sync/web/guide/retrieve-data.html#排序规则) )
...
wilddog.initializeApp(config);
var ref = wilddog.database().ref("/users/jack");
...
wilddog.initializeApp(config);
var ref = wilddog.database().ref("/users/fred/rank");
...
wilddog.initializeApp(config);
var wilmaRef = wilddog.database().ref("/samplechat/users/wilma");
...
wilddog.database().goOffline(); // All local Wilddog instances are disconnected
wilddog.database().goOnline(); // All local Wildodg instances automatically reconnect
```
...
wilddog.initializeApp(config);
var ref = wilddog.database().ref("student");
...
产生一个新`Query`对象，按照当前节点的值进行排序。排序的详情请参考[数据排序](/sync/web/guide/retrieve-data.html#数据排序)。
...
wilddog.initializeApp(config);
var scoresRef = wilddog.database().ref("scores");
...
wilddog.initializeApp(config);
var disconnectRef = wilddog.database().ref("disconnectMessage");
...
wilddog.initializeApp(config);
var fredOnlineRef = wilddog.database().ref("/users/fred/online");
...
wilddog.initializeApp(config);
var sessionsRef = wilddog.database().ref("sessions");
...
wilddog.initializeApp(config);
var ref = wilddog.database().ref("/samplechat/users/jim");
...
wilddog.initializeApp(config);
var ref = wilddog.database().ref("/samplechat/users");
