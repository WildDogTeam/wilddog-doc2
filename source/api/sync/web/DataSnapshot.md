...
exists() 方法比 snapshot.val() !== null 更高效。
...
val() 可能返回的数据类型包括：string、number、boolean、null（该节点下数据为空时） 或者是数组、对象。返回的数据类型取决于节点下的数据格式。

**返回**

`object|string|null|number|boolean` 当前快照的真实数据, null 表示该节点的数据快照为空。
...
通过相对路径来获取字节点的数据快照，相对路径可以是一个字节点的 key 值（如：'Beijing'），也可以是更深层次的路径（如：'Beijing/pm25'）。如果相对路径下并没有数据，则返回 null 。
...
受 JavaScript Object 对象的影响， snapshot 直接通过 val() 方法返回的数据不能保证显示的顺序完全符合 orderBy\*() 的要求，因此我们提供了 forEach() 方法来解决这个问题。<br>如果没有使用 orderBy\*() 方法，依次遍历出来的结果默认选择 key 排序（除非数据快照设置过 priority ,则会根据 priority 排序）。

**参数**

| 参数名      | 说明             |
| -------- | -------------- |
| callback | function(non-null)类型<br>遍历每一个子节时的回调函数。如果在 callback 中返回 true 则会停止之后的遍历。 |
...
query.once("value")
  .then(function(snapshot) {
    snapshot.forEach(function(childSnapshot) {
      // key will be "Jim" the first time and "alan" the second time
...
你可以通过使用 hasChildren() 方法来确定当前的数据快照是否含有多个字节点，进而决定是否调用 forEach() 方法来遍历数据。
...
var ref = wilddog.sync().ref("/city/Beijing");
ref.once('value', function(snapshot){
    console.log(snapshot.val()); // {'pm25',432}
    var newRef = snapshot.ref();
    newRef.once('value', function (newSnapshot) {
        console.log(newSnapshot.val()); // 432
    })
...
导出 `DataSnapshot` 中的内容到 Javascript 对象。
...
exportVal() 方法和 val() 方法类似，如果节点中包含不为空的 priority 值， exportVal() 会导出为适合备份的数据格式。
