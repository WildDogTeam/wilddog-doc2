...
- 通过 [手机登录](/auth/Egret/guide/phone.html) 创建
- 通过 [邮箱登录](/auth/Egret/guide/password.html) 创建
- 通过第三方身份认证提供商授权创建
...
用户信息包含 [用户属性](/auth/Egret/guide/concept.html#用户属性) 及用户的登录信息。
...
### 获取 Provider 的用户属性
 ...
 `providerData` 用于获取所有 [Provider](/auth/Egret/guide/concept.html#Provider) 的用户属性。
...
## 更新用户信息
 `User` 实例用于更新 [用户属性](/auth/Egret/guide/concept.html#用户属性) 及用户的登录信息。
...
<blockquote class="warning">
  <p><strong>注意：</strong></p>
  <ul>
    <li>要更新用户的手机号，该用户必须最近登录过。请参考 [重新进行身份认证](/auth/Egret/guide/manageuser.html#重新进行身份认证)。</li>
...
<blockquote class="warning">
  <p><strong>注意：</strong></p>
  <ul>
    <li>要更新用户的邮箱地址，该用户必须最近登录过。请参考 [重新进行身份认证](/auth/Egret/guide/manageuser.html#重新进行身份认证)。</li>
...
<blockquote class="warning">
  <p><strong>注意：</strong></p>
  <ul>
    <li>要更新密码，该用户必须最近登录过。请参考 [重新进行身份认证](/auth/Egret/guide/manageuser.html#重新进行身份认证)。</li>
...
<blockquote class="warning">
  <p><strong>注意：</strong></p>
  <ul>
    <li>要删除用户，该用户必须最近登录过。请参考[重新进行身份认证](/auth/Egret/guide/manageuser.html#重新进行身份认证)。</li>
    <li>使用 customToken 登录时，若该登录用户为 admin 用户，则不能更新用户密码。</li>
  </ul>
</blockquote>
