...
#### 选择野狗提供的模版
你可以使用野狗提供的验证码模版和验证服务，无需生成验证码。 目前野狗提供两种模版，可分别于`验证号码`和`重置密码`场景。 模版内容由固定内容和一个 6 位验证码组成，验证码由野狗生成和校验。使用过程如下:
...
#### 自定义验证码模版
你也可以自定义验证码模版，需要生成和校验验证码。配置模版内容需要带有关键字`验证码`以及至少一个`%s`。 并且生成的验证码需要是 6 位数字。每个用户可以用 [变量](/guide/sms/vertification.html#配置变量) 来替换部分内容，每个应用最多创建 50 条验证码模版。配置的验证码模版需要提交审核。使用过程如下:
