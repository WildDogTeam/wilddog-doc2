...
## 手动注册离线事件
通过 [离线事件](/sync/Egret/guide/offline-capabilities.html#离线事件) 与 [监听连接状态](/sync/Egret/guide/offline-capabilities.html#监听连接状态) 配合使用，将离线事件的注册代码放到监听连接状态的回调函数中，可实现异常情况下离线事件的重新注册：
