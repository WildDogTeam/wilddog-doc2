...
向指定节点写入数据。此方法会先清空指定节点，再写入数据。详细使用请参考：[set() 完整指南-写入数据](/sync/Egret/guide/save-data.html#写入数据)。
...
对当前节点进行数据合并操作，更新当前节点下的数据。详细使用请参考：[update() 完整指南-更新数据](/sync/Egret/guide/save-data.html#更新数据)。
...
删除当前节点，效果等同于 `set(null)`，如果父级节点只有当前节点一个子节点，会递归删除父级节点。详细使用请参考：[完整指南-删除数据](/sync/Egret/guide/save-data.html#删除数据)。
...
新增子节点的 key 基于时间戳和随机算法生成，并可以按照时间先后进行排序。详细使用请参考：[完整指南-追加字节点](/sync/Egret/guide/save-data.html#追加子节点)。
...
[wilddog.Promise](Promise.html).<[wilddog.sync.Reference](Reference.html)>
...
把数据写到当前位置，类似 [set](Reference.html#set)，不同之处是需要指定一个优先级。默认排序按照优先级排序（参考 [orderByPriority](/sync/Egret/guide/retrieve-data.html#排序规则)）。详细使用请参考：[完整指南-写入数据并设置节点优先级](/sync/Egret/guide/save-data.html#写入数据并设置节点优先级)。
...
设置当前节点的优先级，支持为每个节点设置优先级 (priority)，用于实现节点按优先级排序。优先级是节点的隐藏属性，默认为 `null`。详细使用请参考：[完整指南-设置节点优先级](/sync/Egret/guide/save-data.html#设置节点优先级)。
...
在 `transaction()` 的执行过程中客户端可能会重复写入直到成功，也可以在 `updateFunction` 中直接 `return;` 手动终止事务。详细使用请参考：[完整指南-事务处理](/sync/Egret/guide/save-data.html#事务处理)。
...
获取与当前数据节点关联的离线事件对象。详细使用请参考：[完整指南-离线功能](/sync/Egret/guide/offline-capabilities.html)。
