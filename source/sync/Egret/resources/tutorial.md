...
如需了解创建应用、读写数据等基础操作，请参考文档[快速入门](/sync/Egret/quickstart.html)。
...
使用写入数据的 API [push()](/sync/Egret/guide/save-data.html#追加子节点)，它用来在当前节点下生成随机子节点，以保证 key 的不重复和有序。
...
```
### 4. 在窗口显示
在窗口显示即读取数据，Wilddog [获取数据](/sync/Egret/guide/retrieve-data.html)是先绑定监听事件，然后在回调函数中获取数据：
