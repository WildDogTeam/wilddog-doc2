...
Sync 中，数据在云端发生的任何变化都称为事件。数据的不同变化都有对应的 [事件类型](/sync/Web/guide/retrieve-data.html#事件)。
...
具体相关信息，请参考：[事件监听](/sync/Web/guide/retrieve-data.html) 。
...
具体相关信息，请参考：[安全性与规则](/sync/Web/rules/introduce.html) 。