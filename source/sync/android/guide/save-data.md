...
**特别注意** ：写入数据及其他操作，都要先确保与 WilddogSync 云端长连接建立成功。如果你要复制本文档中的代码示例体验，最简单的方式就是加个 Thread.sleep() 线程等待，以确保长连接的成功建立，请自行在每个代码片段下面加上：
```java
//线程等待，以确保长连接的成功建立
try {
  Thread.sleep(6 * 1000);
...
```java
WilddogSync ref = new WilddogSync("https://samplechat.wilddogio.com/android/saving-data/wildblog");
```
...
WilddogSync usersRef = ref.child("users");
...
```java
WilddogSync usersRef = new WilddogSync("https://samplechat.wilddogio.com/android/saving-data/wildblog/users");
...
```java
var ref = new WilddogSync("https://samplechat.wilddogio.com/android/saving-data");
ref.child("setValue").setValue("hello", new WilddogSync.CompletionListener() {
...
```
绝大多数操作都可设置回调函数来确保操作的完成，具体使用参见 [API文档](/sync/android/api.html)。
...
WilddogSync hopperRef = usersRef.child("gracehop");
...
WilddogSync ref = new WilddogSync("https://<appId>.wilddogio.com/a");
Map<String, Object> children = new HashMap<String, Object>();
...
为了解决这个问题，WilddogSync `push()` 采用了生成唯一ID 作为 key 的方式。通过这种方式，多个用户同时在一个节点下面 push 数据，他们的 key 一定是不同的。这个 key 是通过一个基于时间戳和随机算法生成的，即使在一毫秒内也不会相同，并且表明了时间的先后(可用来排序)，WilddogSync 采用了足够多的位数保证唯一性。

用户可以用 `push` 向[博客应用](https://docs-examples.wilddogio.com/android/saving-data/wildblog)中写新内容：

```java
WilddogSync ref = new WilddogSync("https://docs-examples.wilddogio.com/android/saving-data/wildblog/posts");

WilddogSync newRef1 = ref.push();
newRef1.child("author").setValue("gracehop");
newRef1.child("title").setValue("Announcing COBOL, a New Programming Language");

WilddogSync newRef2 = ref.push();
...
```java
WilddogSync ref = new WilddogSync("https://<appId>.wilddogio.com/removeValue");
// 不带回调    
ref.removeValue();

// 或带回调，效果同上。
ref.removeValue(new WilddogSync.CompletionListener(){
...
**注意**：WilddogSync 不会保存空路径，即如果 /a/b/c 节点下的值被设为 null，这条路径下又没其他的含有非空值的子节点存在，那么云端就会把这条路径删除。
...
```java
var upvotesRef = new WilddogSync('https://docs-examples.wilddogio.com/saving-data/wildblog/posts/-JRHTHaIs-jNPLXOQivY/upvotes');
...
更多使用参见 [transaction()](/sync/android/api.html#runTransaction-Transaction-Handler)。
