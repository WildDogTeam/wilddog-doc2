...
创建一个新 [wilddog.sync.Query](Query.html) 实例，可以查询所有大于或等于指定的 key、value 或 priority 的节点，具体取决于所选的排序方法。详细使用请参考：[完整指南-根据数据筛选结果监听](/sync/微信小程序/guide/retrieve-data.html#根据数据筛选结果监听)。
