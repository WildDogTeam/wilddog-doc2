window.addEventListener('load', function () {
  ...
  var searchFailed = function (word, err) {
    ...
    if (word) {
      getClass('search-result-title')[0].innerHTML = '“<em>' + word + '</em>”的搜索结果';
      getClass('search-tips-text')[0].innerHTML = '抱歉，未找到“<em>' + word + '</em>”的相关结果，请尝试其他关键词搜索';
    } else {
      getClass('search-result-title')[0].innerHTML = '“<em>' + keyword + '</em>”的搜索结果';
      getClass('search-tips-text')[0].textContent = '抱歉，由于网络原因搜索失败，请稍后尝试搜索。';
    }
    getClass('searching')[0].style.display = 'block';
    getClass('search-result-tab')[0].style.display = 'none';
    getClass('help-search-result-list')[0].style.display = 'none';
    getClass('page-control')[0].style.display = 'none';
  ...
  var searchSuccess = function () {
    clearInterval(timer);
    getClass('search-result-title')[0].innerHTML = '“<em>' + keyword + '</em>”的搜索结果，共'+ totalResult +'个';
  ...
  function createResultList (list) {
    ...
    list.forEach(function (ele) {
      ...
      var liContent = '<a href="' + ele.uri + '" class="help-search-result-item-link">' + (ele.title == '' ? ele.uri : ele.title) + '</a><div class="help-search-result-item-text">' + ele.content + '</div>';
      li.innerHTML = liContent;
      [].slice.call(li.getElementsByClassName('help-search-result-item-text')[0].getElementsByTagName('a'), 0).forEach(function (e) {
        e.setAttribute('target', '_self');
        e.setAttribute('href', 'javascript:;');
      })
      getClass('help-search-result-list')[0].appendChild(li);
    })
  ...
  var doSearch = function (keyword, type, page) {
    getClass('search-result-title')[0].innerHTML = '搜索结果中';
    if (searchHistory[type][page]) {
      createResultList(searchHistory[type][page]);
